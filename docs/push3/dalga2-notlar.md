...
- (İnceleme sonrası) Drum vuruşları P3.drums.trigger(pad, vel, when, 'seq') ile planlanır; bus 'panic' (overlay, blur, mod değişimi) bunlara dokunmaz, yalnız canlı pad seslerini keser. Aşağıdaki Stop davranışı argümansız P3.drums.panic() ile sürer.
